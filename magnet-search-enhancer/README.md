...
`.user.js` 文件同时作为源代码和可直接分发的安装文件。第一类包含熊猫、SkrBT、老王和 Lemon 等使用相似 Bootstrap 结构的网站；BT4G 为第二类；使用 Zsky 资源卡片结构的网站为第三类。

“自动获取磁力链接”设置通过 `GM_getValue` 和 `GM_setValue` 保存，因此同一设置可以在第一类和第三类网站之间共享。每次修改脚本行为后都应递增 `@version`，以便用户脚本管理器和 Greasy Fork 检测更新。
