...
- 过滤已知的外部视频广告结果。
- 按链接域名或“在线”站外链接特征，过滤 SkrBT 列表和 Bootstrap 结果卡片中的推广结果。
- 为搜索结果增加编号和可配置的本页二次筛选功能，默认不使用二次筛选，关闭时不显示筛选输入框。
- 支持折叠或展开结果中的文件列表。
- 将容易误解的结果链接改为“查看详情”。
...
“使用二次筛选”位于右下角配置面板，默认关闭，并在 SkrBT 与其他 Bootstrap 搜索站之间共享设置。关闭时脚本不显示本页筛选输入框，也不会隐藏正常结果；开启后可在输入框中输入关键词进一步筛选当前页。
